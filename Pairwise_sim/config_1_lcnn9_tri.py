...
# -*- coding: utf-8 -*-
"""
Created on Fri Jun  5 21:22:57 2020
...
class Configuration(object):
    ...
    project_dir = Path(__file__).resolve().parents[0] # Current directory
    root_dir = Path(__file__).resolve().parents[1]
    dataset_dir=Path(__file__).resolve().parents[5] / 'Composite2Photo'
    # </editor-fold>  

    # <editor-fold desc="[+] Pre-trained Model">
#    vgg_face_dag_pth = project_dir / 'pre_trained_models' / 'vgg_face_dag.pth'  # weights of the pre-trained model
#    senet50_256_pth = project_dir / 'pre_trained_models' / 'senet50_256.pth'
   ...
   # <editor-fold desc="[+] Summury Path">
    train_summary_dir = project_dir /'train_val_test_summary'/ 'train_summary'
    valid_summary_dir = project_dir / 'train_val_test_summary'/ 'valid_summary'
    test_summary_dir = project_dir / 'train_val_test_summary'/ 'test_summary'   
    # </editor-fold>
...
# <editor-fold desc="[+] Trained Model Path">
    ...
    if restore_checkpoint == False:
        ...
        trained_model_dir = project_dir / os.path.join('log_results', 'log_results_Wed_06Oct2021_1535_Malta_SetA_anchor_sketch_type') / 'ckt'
    ...
    config_fname_='log_results_'+date_time+ '/'+os.path.basename(__file__) 
    config_fname=os.path.basename(__file__) 
    config_fname_=config_fname.split('.',1)[0]
    save_results_dir= project_dir / os.path.join('log_results', 'log_results_'+date_time) 
    load_setB = False
...
#    pretrain=True
    ...
    if (not do_cross_validation) and fixed_testset: #For Fixed training and test set
        ...
        # train_meta_file = os.path.join(dataset_dir, 'GT','Norm_Sorted_Set_test_rm1.txt')# For Grad-CAM training using triplet loss
        ...
        if (testdata == "UoM"): 
            ...
            test_meta_file = os.path.join(dataset_dir, 'GT', 'Norm_Sorted_Set_test_lcnn9_GT_as_Rank1.txt') # Rank1 as GT
            # test_meta_file = os.path.join(dataset_dir, 'GT','Hiding_game','lcnn9','triplet_gain','Random_txt_files','Norm_Sorted_Set_test_lcnn9_GT_as_Rank1_99percent_hidden.txt') # Rank1 as GT
        elif (testdata == "VM"):
...
#    lr_scheduler_type = 'ReduceLROnPlateau' 
    ...
    lr_decay_rate=0.95 # decay rate # Similar to Yan's training on triplet loss
    weight_decay = 0
    # weight_decay = 1e-4
    # n_epochs = 30 # Number of training epochs
    n_epochs = 30 # Number of training epochs  # Similar to Yan's training on triplet loss
    start_epoch = 0 # From which epoch to resume training which is set by Solver._resume()
#    restore_checkpoint = True # If set true, you need to set checkpoint_file below to a valid path
    checkpoint_file = project_dir /'log_results'/'log_results_Wed_06Oct2021_1535_Malta_SetA_anchor_sketch_type'/'ckt'/'lcnn9_tri_Wed_06Oct2021_173415_epoch30.pth'
    ...
    if (bm_alone == False and bm_pi == False and bm_pi_mis== False):
        n_bclass = 32 # Number of classes/identities in one mini-batch  
    ...
    # lcnn9_tri_pth=None   # Update with the pth file, to directly test the trained model                   #######
    ...
    # n_class = 3227 #2420 #(for ce) #2337  # 3260 # number of classes/identities in the VM dataset
    ...
    # valid_set_size = 0.0 # Proportion of the training data for validation
    ...
    transfer_learning_opt = 1 # 1 - finetune all layers
    ...
    # tri_loss_param = {'margin': 1, 'metric': 'euclidean'} # for the triplet loss # Only if Ratings are considered during training
    tri_loss_param = {'margin': 0.1, 'metric': 'euclidean'} # for the triplet loss # Only if Ratings are not considered during training
...
# from dataset import MVMLoader, SMVMLoader, set_config as set_config_dataset  # VM dataset
...
# from lcnn9_tri_pretrained import get_model, set_config as set_config_net  # Use this for pre-trained model as well
from lcnn9_tri import get_model, set_config as set_config_net  # For visualization
# from lcnn9_tri_vis import get_model, set_config as set_config_net # For visualization via Grad-CAM
...
model = get_model()
model.to(C.device)
#summary(model, input_size=(3, 224, 224), batch_size=-1)  # vgg model
