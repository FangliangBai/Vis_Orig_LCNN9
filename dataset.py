...
class MUoMSGFSLoader(object):
    ...
    def __init__(self, test_only=False):
        ...
        self.train_loader, self.valid_loader, self.valid_acc_loader = None, None, None
        test_cls = None
        if not test_only:
            num_class= self.dataset.n_class()    # 467
            classes = self.dataset.label_set
    ...
    def _get_dataset(self):
        train_dataset, valid_acc_dataset = None, None
...
class InpaintedUoMDataset(Dataset):
    """
    Inpainted UoMSGFS dataset
    """
    ...
    def __init__(self, data_frame):
        ...
        # The following initialization is for BalancedBatchSampler or its kind
        if self.frame.shape[1] > 1:
            self.labels = self.frame.iloc[:,1].astype('int').to_numpy()    # array([2,2,2,...526,526,526]) len(self.labels)=1440
            self.labels = self.labels.tolist()
            self.label_set = sorted(set(self.labels), key=self.labels.index)
        
    def __getitem__(self, index):
        index = index % len(self)
        original_probe = self.transform(Image.open(self.filenames[index * 6]))
        original_mate = self.transform(Image.open(self.filenames[index * 6 + 1]))
        inpainted_probe = self.transform(Image.open(self.filenames[index * 6 + 2]))
        inpainted_mate = self.transform(Image.open(self.filenames[index * 6 + 3]))
        label_transform = transforms.ToTensor()
        inpainted_probe_mask = label_transform(Image.open(self.filenames[index * 6 + 4]))
        inpainted_mate_mask = label_transform(Image.open(self.filenames[index * 6 + 5]))
        
        return original_probe, original_mate, inpainted_probe, inpainted_mate, \
            inpainted_probe_mask, inpainted_mate_mask
    
    def __len__(self):
        return int(len(self.filenames) / 6)
