...
class WhiteboxLightCNN9(WhiteboxNetwork):
    ...
    def set_triplet_classifier(self, x_mate, x_nonmate):
        ...
        passed_fc_w = fc_w[indicator, :]
        passed_fc_b = fc_b[indicator]

        embedding_weights = torch.cat((x_mate, x_nonmate), dim=0)
        new_fc_w = torch.matmul(embedding_weights, passed_fc_w)
        new_fc_b = torch.matmul(embedding_weights, passed_fc_b)
        self.net.fc = nn.Linear(8192, 2)
        self.net.fc.weight = nn.Parameter(new_fc_w)
        self.net.fc.bias = nn.Parameter(new_fc_b)
        # Modify fc2 layer
...
class WhiteboxVGG16(WhiteboxNetwork):
    ...
    def set_triplet_classifier(self, x_mate, x_nonmate):
        ...
        # indicator = self.net.fc.indicator.bool()
        model_weights = list(self.net.parameters())
        fc_w = model_weights[-3]
        fc_b = model_weights[-2]

        embedding_weights = torch.cat((x_mate, x_nonmate), dim=0)
        new_fc_w = torch.matmul(embedding_weights, fc_w)
        new_fc_b = torch.matmul(embedding_weights, fc_b)
        self.net.fc = nn.Linear(25088, 2)
        self.net.fc.weight = nn.Parameter(new_fc_w)
        self.net.fc.bias = nn.Parameter(new_fc_b)
        # Modify fc2 layer
        self.net.fc2 = nn.Identity(2)
