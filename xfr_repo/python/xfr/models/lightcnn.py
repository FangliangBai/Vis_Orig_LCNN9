...
class mfm(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.mode == 0:
            ...
            self.indicator = torch.cat((1-arg, arg), dim=0)
...
def LightCNN_9Layers(**kwargs):
    ...
    ckpt = torch.load("/media/kent/DISK2/E2ID/local_fbai/E2ID_Q7_Refined_2/pre_trained_models/LightCNN_9Layers_checkpoint.pth.tar")
    model = nn.DataParallel(model)
    model.load_state_dict(ckpt['state_dict'])
    model = model.module
    model.training = False
    print('[INFO] Pre-trained model loaded: LightCNN_9Layers. ')
    return model
