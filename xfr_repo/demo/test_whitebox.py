...
sys.path.append('../xfr_repo/python')
...
import xfr.models.lcnn9_tri
import xfr.show

sys.path.append('../xfr_repo/python/strface')
...
def _encode_triplet_test_cases(wb, img_urls, mask=None):
    ...
    if mask is None:
        ...
        # f_probe = '../data/vm_data/EBPVis_im/triplet_452_probe.png'
        # f_mate = '../data/vm_data/EBPVis_im/triplet_452_mate.png'
        # f_nonmate = '../data/vm_data/EBPVis_im/triplet_452_non-mate.png'
        f_probe, f_mate, f_nonmate = img_urls
    elif mask == 'nose':
    ...
    im_probe = vipy.image.ImageDetection(filename=f_probe)
    # im_nonmate = vipy.image.ImageDetection(filename=f_nonmate)
    # im_mate = vipy.image.ImageDetection(filename=f_mate)
    img_probe_display = im_probe.clone().resize(112,112).rgb().numpy()  # numpy image

    # x_mate = wb.net.encode(wb.net.preprocess(im_mate.pil()))
    # x_nonmate = wb.net.encode(wb.net.preprocess(im_nonmate.pil()))
    # img_probe = wb.net.preprocess(im_probe.pil())

    im_probe = PIL.Image.open(f_probe)
    im_nonmate = PIL.Image.open(f_nonmate)
    im_mate = PIL.Image.open(f_mate)

    x_mate = wb.net.encode(wb.net.preprocess(im_mate))
    x_nonmate = wb.net.encode(wb.net.preprocess(im_nonmate))
    img_probe = wb.net.preprocess(im_probe)  # torch tensor

    return (x_mate, x_nonmate, img_probe, img_probe_display)

def _encode_triplet_test_cases_vgg16(wb, img, mask=None):
    """Run face detection and network encoding for standardized (mate, non-mate, probe) triplets for testing"""
    if mask is None:
        # f_probe = '../data/n00000001_00000117.JPEG'
        # f_nonmate = '../data/n00000002_00000100.JPEG'
        ...
        # f_mate = '../data/vm_data/EBPVis_im/triplet_452_mate.png'
        # f_nonmate = '../data/vm_data/EBPVis_im/triplet_452_non-mate.png'
        f_probe, f_mate, f_nonmate, probe_url = img
    elif mask == 'nose':
        f_probe = '../data/inpainting-game/IJBC/aligned/8/img/1017/inpainted/00002_truth.png'   # obama
        f_nonmate = '../data/inpainting-game/IJBC/aligned/8/img/1017/inpainted/00002_out_0.png'  # obama non-mate: inpainted probe
        f_mate = '../data/inpainting-game/IJBC/aligned/8/img/1406/inpainted/00002_truth.png'  # obama mate
    ...
    # im_mate = vipy.image.ImageDetection(filename=f_mate).boundingbox(xmin=bb_mate[0], ymin=bb_mate[1], width=bb_mate[2], height=bb_mate[3]).dilate(1.1).crop().mindim(256).centercrop(224, 224)
    # img_probe_display = im_probe.clone().resize(112,112).rgb().numpy()  # numpy image
    im_probe = vipy.image.ImageDetection(filename=probe_url)
    img_probe_display = im_probe.clone().resize(112,112).rgb().numpy()
    x_mate = wb.net.encode(f_mate)
    x_nonmate = wb.net.encode(f_nonmate)

    return (x_mate, x_nonmate, f_probe, img_probe_display)
...
def triplet_ebp_lightcnn9():
    """Triplet excitation backprop"""
    print('[test_whitebox.triplet_ebp]: Detection and encoding for (mate, probe)')

    net = xfr.models.lcnn9_tri.LCNN9EmbeddingNet()
    ckpt = torch.load("/media/kent/DISK2/E2ID/local_fbai/E2ID_Q7_Refined_2/pre_trained_models/lcnn9_tri_bestacc_2.pth")
    net.load_state_dict(ckpt["model_state_dict"], strict=False)
    net.eval()
    wb = xfr.models.whitebox.Whitebox(xfr.models.whitebox.WhiteboxLightCNN9(net))


    (x_mate, x_nonmate, img_probe, img_probe_display) = _encode_triplet_test_cases(wb)
    wb.net.set_triplet_classifier((1.0/2500.0)*x_mate, (1.0/2500.0)*x_nonmate)  # rescale encodings to avoid softmax overflow
    P = torch.zeros( (1, wb.net.num_classes()) );  P[0][0] = 1.0;  # one-hot prior probability for mate
    img_saliency = wb.ebp(img_probe, P)
    outfile = './test_whitebox_triplet_ebp_lightcnn9.jpg';
    _blend_saliency_map(img_probe_display, img_saliency).save(outfile)
    print('[test_whitebox.triplet_ebp]: saving saliency map blended overlay to "%s"' % outfile)

def triplet_ebp_lightcnn9_vmeval(probe, mate, nonmate):
    """Triplet excitation backprop"""
    print('[test_whitebox.triplet_ebp]: Detection and encoding for (mate, probe)')
    ...
    (x_mate, x_nonmate, img_probe, img_probe_display) = _encode_triplet_test_cases(wb, (probe, mate, nonmate))
    wb.net.set_triplet_classifier((1.0/2500.0)*x_mate, (1.0/2500.0)*x_nonmate)  # rescale encodings to avoid softmax overflow
    P = torch.zeros( (1, wb.net.num_classes()) );  P[0][0] = 1.0;  # one-hot prior probability for mate
    img_saliency = wb.ebp(img_probe, P)
    outfile = './test_whitebox_triplet_ebp_lightcnn9.jpg';
    blend_saliency_map = _blend_saliency_map(img_probe_display, img_saliency)
    print('[test_whitebox.triplet_ebp]: saving saliency map blended overlay to "%s"' % outfile)

    return img_saliency, blend_saliency_map
...
def contrastive_triplet_ebp_lightcnn9():
    """Contrastive triplet excitation backprop"""
    print('[test_whitebox.contrastive_triplet_ebp]: Detection and encoding for (mate, non-mate, probe) triplet')
    ...
    wb.net.set_triplet_classifier((1.0/2500.0)*x_mate, (1.0/2500.0)*x_nonmate)  # rescale encodings to avoid softmax overflow
    img_saliency = wb.contrastive_ebp(img_probe, k_poschannel=0, k_negchannel=1)
    outfile = './test_whitebox_contrastive_triplet_ebp_lightcnn9.jpg';
    _blend_saliency_map(img_probe_display, img_saliency).save(outfile)
    print('[test_whitebox.contrastive_triplet_ebp]: saving saliency map blended overlay to "%s"' % outfile)

def contrastive_triplet_ebp_lightcnn9_vmeval(probe, mate, nonmate):
    """Contrastive triplet excitation backprop"""
    print('[test_whitebox.contrastive_triplet_ebp]: Detection and encoding for (mate, non-mate, probe) triplet')
...
def truncated_contrastive_triplet_ebp_lightcnn9():
    """Truncated contrastive triplet excitation backprop"""
    print('[test_whitebox.truncated_contrastive_triplet_ebp]: Detection and encoding for (mate, non-mate, probe) triplet')

    net = xfr.models.lcnn9_tri.LCNN9EmbeddingNet()
    ckpt = torch.load("/media/kent/DISK2/E2ID/local_fbai/E2ID_Q7_Refined_2/pre_trained_models/lcnn9_tri_bestacc.pth")
    net.load_state_dict(ckpt["model_state_dict"], strict=False)
    net.eval()
    wb = xfr.models.whitebox.Whitebox(xfr.models.whitebox.WhiteboxLightCNN9(net))
    ...
    wb.net.set_triplet_classifier((1.0/2500.0)*x_mate, (1.0/2500.0)*x_nonmate)  # rescale encodings to avoid softmax overflow
    img_saliency = wb.truncated_contrastive_ebp(img_probe, k_poschannel=0, k_negchannel=1, percentile=20)
    outfile = './test_whitebox_truncated_contrastive_triplet_ebp_lightcnn9.jpg';
    _blend_saliency_map(img_probe_display, img_saliency).save(outfile)
    print('[test_whitebox.truncated_contrastive_triplet_ebp]: saving saliency map blended overlay to "%s"' % outfile)

def truncated_contrastive_triplet_ebp_lightcnn9_vmeval(probe, mate, nonmate):
    """Truncated contrastive triplet excitation backprop"""
    print('[test_whitebox.truncated_contrastive_triplet_ebp]: Detection and encoding for (mate, non-mate, probe) triplet')
...
def triplet_ebp_vgg16_vmeval(net, probe, mate, nonmate, probe_url):
    # net = xfr.models.lcnn9_tri.LCNN9EmbeddingNet()
    # ckpt = torch.load("/media/kent/DISK2/E2ID/local_fbai/E2ID_Q7_Refined_2/pre_trained_models/lcnn9_tri_bestacc_2.pth")
    # net.load_state_dict(ckpt["model_state_dict"], strict=False)
    # net = get_model('/media/kent/DISK2/E2ID/Visualization/Pairwise_sim/log_results/Final_log_results_Mon_11Oct2021_1755_Malta_SetA_anchor_sketch_type/ckt/vgg_tri_2_Mon_11Oct2021_221223_epoch30.pth')

    from Pairwise_sim.vgg_tri_2_vis_2 import get_model, set_config as set_config_net
    model = get_model()
    model.to('cuda')
    model.eval()
    wb = xfr.models.whitebox.Whitebox(xfr.models.whitebox.WhiteboxVGG16(model))

    (x_mate, x_nonmate, img_probe, img_probe_display) = _encode_triplet_test_cases_vgg16(wb, (probe, mate, nonmate, probe_url))
    wb.net.set_triplet_classifier((1.0/2500.0)*x_mate, (1.0/2500.0)*x_nonmate)  # rescale encodings to avoid softmax overflow
    P = torch.zeros( (1, wb.net.num_classes()) ).to('cuda:0');  P[0][0] = 1.0;  # one-hot prior probability for mate
    img_saliency = wb.ebp(img_probe, P)
    outfile = './test_whitebox_triplet_ebp_lightcnn9.jpg';
    blend_saliency_map = _blend_saliency_map(img_probe_display, img_saliency)
    print('[test_whitebox.triplet_ebp]: saving saliency map blended overlay to "%s"' % outfile)
...
def contrastive_triplet_ebp_vgg16_vmeval(net, probe, mate, nonmate, probe_url):
    """Contrastive triplet excitation backprop"""
    from Pairwise_sim.vgg_tri_2_vis_2 import get_model, set_config as set_config_net
    model = get_model()
    model.to('cuda')
...
def truncated_contrastive_triplet_ebp_vgg16_vmeval(net, probe, mate, nonmate, probe_url):
    from Pairwise_sim.vgg_tri_2_vis_2 import get_model, set_config as set_config_net
    model = get_model()
    model.to('cuda')
    model.eval()
